...
export interface VoiceCommercialInput {
  assetId: string;
  repoName: string;
...
export interface VoiceCommercialAgentDeps {
  fal: FalMediaClient;
  elevenLabs: ElevenLabsClient;
  writer: GenerateWrittenAsset;
}

export function makeGenerateVoiceCommercialAsset(
  deps: VoiceCommercialAgentDeps
) {
  return async function generateVoiceCommercialAsset(
    ...
    const audio = await deps.elevenLabs.synthesizeSingleVoice({
      cacheKey: audioCacheKey,
      text: writerResult.content,
    });

    const heroImage = await deps.fal.generateImage(
...
export type GenerateVoiceCommercialAsset = ReturnType<
  typeof makeGenerateVoiceCommercialAsset
>;
