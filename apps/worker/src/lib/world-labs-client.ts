...
/**
 * Worker-side World Labs (Marble) client.
 *
 ...
 * `generateWorldScene` blocks the BullMQ generation worker for the
 * duration of that cycle. The worker concurrency is set high enough
 * (and the World Labs key is rare enough in practice) that no
 * back-pressure tuning is needed beyond the existing queue limits.
 *
 * Boundary discipline
 * -------------------
 *
 * Every HTTP response goes through a Zod schema in
 * `apps/worker/src/lib/schemas/world-labs.ts` before any field is
 * dereferenced. The schemas use `.passthrough()` so the upstream can
 * add fields without breaking the worker, but the fields the worker
 * actually reads (`done`, `response.id`, `response.world_marble_url`,
 * etc.) are validated up front and the parse failure naming the
 * missing field is what bubbles up — not a confusing chained-optional
...
function requireApiKey(): string {
  ...
  if (!apiKey) {
    throw new Error(
      'World Labs world generation requires WORLD_LABS_API_KEY to be set on the worker environment'
    );
  }
  return apiKey;
}
...
async function pollOperation(
  ...
  const timeoutMs = env.WORLD_LABS_POLL_TIMEOUT_SECONDS * 1000;
  const intervalMs = env.WORLD_LABS_POLL_INTERVAL_SECONDS * 1000;
  ...
  throw new Error(
    `World Labs operation ${operationId} did not complete within ${env.WORLD_LABS_POLL_TIMEOUT_SECONDS}s`
  );
}

async function fetchCanonicalWorld(
  apiKey: string,
  worldId: string
): Promise<World> {
...
function buildPublicMarbleUrl(worldId: string): string {
  ...
  // worker does not need to construct it from the (sometimes-null)
 ...
 * normalise the result into a single object the worker can write to
 * the asset row. Throws on every documented failure mode (missing
 * key, HTTP error, schema mismatch, polling timeout, operation error)
 * with a structured message naming the failure surface.
 */
export async function generateWorldScene(
  input: WorldLabsGenerateInput
): Promise<WorldLabsGenerateResult> {
  const apiKey = requireApiKey();

  const startOperation = await startGeneration(apiKey, input);
  console.log(
  ...
  // Re-validate the snapshot through `WorldSchema` so an
  // `as` cast is unnecessary at the dereference points below. Even
  // though `OperationSchema.response` is already validated, the
  // explicit re-parse here documents the boundary and keeps the
  // `World` type narrowing in one place.
  ...
  if (!canonicalUrl) {
    ...
    try {
      const canonical = await fetchCanonicalWorld(apiKey, world.id);
