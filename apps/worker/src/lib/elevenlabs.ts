...
/**
 * Worker-side ElevenLabs client for Phase 4 audio synthesis.
 *
 * Two synthesis surfaces:
 *   1. Single-voice 30s commercial (`synthesizeSingleVoice`).
 *   2. Multi-speaker 2–3 minute podcast (`synthesizeMultiVoiceDialogue`).
 *
 * Both write MP3s into a shared cache directory that the dashboard's
 * streaming route in `apps/web` reads back. The directory name is
 * intentionally distinct from the web narration cache
 * (`.cache/elevenlabs`) so the two clients never collide on filenames
 * or evict each other's outputs.
 */
...
const ELEVENLABS_RENDER_CACHE_DIR = path.resolve(
  process.cwd(),
...
export function getWorkerElevenLabsConfig(): {
  apiKey: string;
  ...
  return {
    apiKey,
    primaryVoiceId,
    altVoiceId: env.ELEVENLABS_VOICE_ID_ALT ?? primaryVoiceId,
    modelId: env.ELEVENLABS_MODEL_ID ?? null,
  };
}

export function buildAudioCacheKey(seed: string): string {
  return createHash('sha1').update(seed).digest('hex').slice(0, 16);
}

function getAudioPath(cacheKey: string): string {
  return path.join(ELEVENLABS_RENDER_CACHE_DIR, `${cacheKey}.mp3`);
}
...
function requireConfig(): {
  ...
  const config = getWorkerElevenLabsConfig();
  if (!config) {
    throw new Error(
      'Worker ElevenLabs synthesis requires ELEVENLABS_API_KEY and ELEVENLABS_VOICE_ID'
    );
  }
  return config;
}
...
async function writeAtomic(targetPath: string, buffer: Buffer): Promise<void> {
  await mkdir(ELEVENLABS_RENDER_CACHE_DIR, { recursive: true });
  const tempPath = buildTempPath(path.basename(targetPath, '.mp3'));
  await writeFile(tempPath, buffer);
  await rename(tempPath, targetPath);
}

export async function synthesizeSingleVoice(input: {
  cacheKey: string;
  text: string;
...
export async function synthesizeMultiVoiceDialogue(input: {
  cacheKey: string;
