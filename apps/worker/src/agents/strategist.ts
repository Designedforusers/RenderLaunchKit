  ...
  "assetsToGenerate": [
    {
      ...
      "brief": "specific instructions for generating this asset",
      "priority": 1-5
    }
  ],
  "skipAssets": [
...
Based on this information, create a focused, opinionated strategy. Don't try to be everywhere — pick the channels and assets that will have the highest impact for this specific product.`;
  ...
  if (!strategy.positioning || !strategy.tone || !strategy.assetsToGenerate) {
  ...
  if (!assetTypes.includes('blog_post')) {
    strategy.assetsToGenerate.push({
      type: 'blog_post',
      brief: 'Write a technical blog post introducing the product, its key features, and why developers should care.',
      priority: 1,
    });
  }
  if (!assetTypes.includes('og_image')) {
    strategy.assetsToGenerate.push({
      type: 'og_image',
      brief: 'Create an OG image that communicates the product value at a glance.',
      priority: 2,
    });
  }
