 ...
 */
export async function aggregateLearnings(): Promise<void> {
  console.log('[Cron:Learn] Aggregating learnings from user feedback...');
  ...
  if (projectsWithFeedback.length === 0) {
    console.log('[Cron:Learn] No projects with user feedback found');
    return;
  }
  ...
  console.log(`[Cron:Learn] Generated/updated ${insightCount} insights from ${projectsWithFeedback.length} projects`);
}
